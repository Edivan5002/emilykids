...
const Marcas = () => {
  ...
  const fetchMarcas = async () => {
    ...
    try {
      ...
      const marcasData = extractData(response);
  ...
  return (
    <div className="page-container" data-testid="marcas-page">
      ...
      <h3 className="text-lg font-semibold text-gray-900 mb-4 mt-6">
        Lista de Marcas - {filteredMarcas.length} {filteredMarcas.length === 1 ? 'registro' : 'registros'}
      </h3>
      ...
      {filteredMarcas.length > ITENS_POR_PAGINA && (
        <div className="mt-4 p-4 border rounded-lg bg-white">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Página {paginaAtual} de {totalPaginas} | Mostrando {indiceInicial + 1} a {Math.min(indiceFinal, filteredMarcas.length)} de {filteredMarcas.length} marcas
            </div>

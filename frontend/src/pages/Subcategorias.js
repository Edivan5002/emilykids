...
const Subcategorias = () => {
  ...
  // Lógica de paginação
  const totalPaginas = Math.ceil(filteredSubcategorias.length / ITENS_POR_PAGINA);
  const indiceInicial = (paginaAtual - 1) * ITENS_POR_PAGINA;
  const indiceFinal = indiceInicial + ITENS_POR_PAGINA;
  const subcategoriasPaginadas = filteredSubcategorias.slice(indiceInicial, indiceFinal);
  ...
  return (
    <div className="page-container" data-testid="subcategorias-page">
      ...
      {filteredSubcategorias.length > ITENS_POR_PAGINA && (
        <div className="mt-4 p-4 border rounded-lg bg-white">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Página {paginaAtual} de {totalPaginas} | Mostrando {indiceInicial + 1} a {Math.min(indiceFinal, filteredSubcategorias.length)} de {filteredSubcategorias.length} subcategorias
            </div>

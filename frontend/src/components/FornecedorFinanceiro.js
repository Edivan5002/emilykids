...
import { DollarSign, TrendingUp, TrendingDown, AlertCircle, Calendar, CreditCard, Activity, PieChart } from 'lucide-react';

const FornecedorFinanceiro = ({ fornecedorId, fornecedorNome }) => {
  ...
  const fetchDadosFinanceiros = async () => {
    ...
    try {
      ...
      const [financeiroResponse, contasResponse] = await Promise.all([
        axios.get(`${backendUrl}/api/fornecedores/${fornecedorId}/financeiro`, { headers }),
        axios.get(`${backendUrl}/api/fornecedores/${fornecedorId}/contas-pagar`, { headers })
      ]);
  ...
  const { resumo, score, metricas, formas_pagamento, categorias_despesa, historico } = dadosFinanceiros;

  return (
    <div className="space-y-6">
      {/* Score de Confiabilidade */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold mb-4">Score de Confiabilidade</h3>
        <div className="flex items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">Taxa de Atraso:</span>
                <span className="font-semibold text-red-600">{metricas.taxa_atraso}%</span>
              </div>
              <div className="flex justify-between">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <div className="text-2xl font-bold text-blue-600">
            {formatCurrency(resumo.total_comprado)}
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {Object.entries(categorias_despesa).map(([categoria, valor]) => (
            <div key={categoria} className="p-3 bg-gray-50 rounded">
              <div className="text-sm text-gray-600 capitalize mb-1">{categoria.replace('_', ' ')}</div>
              <div className="font-semibold text-lg">{formatCurrency(valor)}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Formas de Pagamento */}
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold mb-4">Formas de Pagamento Utilizadas</h3>
            ...
            <div key={conta.id} className="border rounded p-3 hover:bg-gray-50">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                ...
                <div>
                  ...
                  <div className="font-semibold text-green-600">{formatCurrency(conta.valor_pago)}</div>
                </div>
                <div>
              ...
              {conta.parcelas && conta.parcelas.length > 1 && (
                <div className="mt-2 pt-2 border-t">
                  <div className="text-xs text-gray-500">
                    {conta.parcelas.filter(p => p.status === 'pago').length} de {conta.parcelas.length} parcelas pagas
                  </div>
                </div>
              )}
            </div>
...
export default FornecedorFinanceiro;

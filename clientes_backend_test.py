...
BACKEND_URL = "https://inactive-supplier-ui.preview.emergentagent.com/api"
    ...
    def authenticate(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log_test("Authentication", False, f"Login failed with {login_data['email']}: {response.status_code} - {response.text}")
                return False
        except Exception as e:
            self.log_test("Authentication", False, f"Login error with {login_data['email']}: {str(e)}")
            return False
